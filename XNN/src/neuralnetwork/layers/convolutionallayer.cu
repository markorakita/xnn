...
ConvolutionalLayer::ConvolutionalLayer(ParallelismMode parallelismMode, cudaStream_t deviceCalculationStream, cudaStream_t deviceMemoryStream,
	curandState* curandStatesBuffer, uint indexInTier, uint tierSize, uint inputNumChannels, uint inputDataWidth, uint inputDataHeight,
	uint inputDataCount, bool holdsInputData, uint numFilters, uint filterWidth, uint filterHeight, uint numFilterChannels, float filtersUpdateMomentum,
	float filtersUpdateDecay, float filtersUpdateLearningRateProgressStep, float filtersUpdateStartingLearningRate, float filtersUpdateLearningRateUpdateFactor,
	...
	float biasesUpdateLearningRateUpdateFactor, int paddingX, int paddingY, uint stride, ActivationType activationType, float activationAlpha,
	bool holdsActivationGradients)
	:
	WeightsLayer(indexInTier, (size_t)numFilters * filterWidth * filterHeight * numFilterChannels * sizeof(float), inputNumChannels * filterWidth * filterHeight,
		...
		biasesUpdateLearningRateUpdateFactor, activationType, activationAlpha, curandStatesBuffer)
{
	...
	m_weightsGradientsPerChunkBuffer = NULL;
	m_biasesGradientsPartialSumsBuffer = NULL;
...
void ConvolutionalLayer::AllocateBuffers(bool allocateTrainBuffers)
{
	...
	if (allocateTrainBuffers)
	{
		...
		// Allocating buffer for holding partial sums for calculating filters gradients.
		m_preactivationGradientsPerChunkWidth = m_inputNumChannels > 3 ? 3 : 4;
		size_t filtersGradientsPerChunkBufferSize = (size_t)DivideUp(m_numPatchesX, m_preactivationGradientsPerChunkWidth) *
			DivideUp(m_numPatchesY, m_preactivationGradientsPerChunkWidth) * m_weightsBufferSize;
		CudaAssert(cudaMalloc<float>(&m_weightsGradientsPerChunkBuffer, filtersGradientsPerChunkBufferSize));
		m_memoryConsumptionSize += filtersGradientsPerChunkBufferSize;

		// Allocating buffer for holding partial sums for calculating biases gradients.
		m_biasesGradientsPartialSumBlocks = DivideUp(DivideUp(m_inputDataCount * m_numPatchesY * m_numPatchesX, c_biasesGradientsSumsPerThread),
			c_biasesGradientsPartialSumThreadsPerBlock);
		size_t biasesGradientsPartialSumsBuffeSize = (size_t)m_biasesGradientsPartialSumBlocks * c_biasesGradientsPartialSumThreadsPerBlock * m_biasesBufferSize;
		CudaAssert(cudaMalloc<float>(&m_biasesGradientsPartialSumsBuffer, biasesGradientsPartialSumsBuffeSize));
		m_memoryConsumptionSize += biasesGradientsPartialSumsBuffeSize;
...
{
	if (m_weightsGradientsPerChunkBuffer != NULL)
	{
		CudaAssert(cudaFree(m_weightsGradientsPerChunkBuffer));
	}
	if (m_biasesGradientsPartialSumsBuffer != NULL)
	{
		...
		else if (m_inputNumChannels == 3)
		{
			if (m_numFilters % 64 == 0)
			{
				LAUNCH_KERNEL_ASYNC((ApplyFiltersOnInputData<32, 1, 4, 16, 3, 4, true>), gridDimensions, blockDimensions, m_deviceCalculationStream)(m_inputDataBuffer, m_inputDataWidth,
					m_inputDataHeight, m_inputDataSize, m_inputDataCount, m_paddingX, m_paddingY, m_weightsBuffer, m_filterWidth, m_filterHeight, m_filterSize, m_numFilters,
...
void ConvolutionalLayer::CalculateBiasesGradients()
{
	...
	const uint numElementsToSum = m_inputDataCount * m_numPatchesY * m_numPatchesX;
	...
	LAUNCH_KERNEL_ASYNC(__CalculateBiasesGradientsPartialSums, gridDimensions, blockDimensions, m_deviceCalculationStream)(m_preactivationGradientsBuffer,
		numElementsToSum, m_biasesGradientsPartialSumsBuffer);
	CudaAssert(cudaGetLastError());
...
void ConvolutionalLayer::CalculateWeightsGradients()
{
		...
		else if (m_inputNumChannels == 3)
		{
			if (m_numFilters % 64 == 0)
			{
				LAUNCH_KERNEL_ASYNC((CalculateInputDataWeightsGradients<16, 4, 16, 2, 32, 2, 3, true>), gridDimensions, blockDimensions, m_deviceCalculationStream)(m_inputDataBuffer,
					...
					m_filterSize, m_numFilters, m_stride, m_numPatchesX, m_numPatchesY, m_preactivationGradientsPerChunkWidth, m_weightsGradientsPerChunkBuffer);
			}
			else if (m_numFilters % 48 == 0)
			{
			...
			if (m_inputNumChannels % 64 == 0)
			{
				if (m_numFilters % 128 == 0)
				{
					LAUNCH_KERNEL_ASYNC((CalculateFilteredDataWeightsGradients<32, 4, 8, 8, 16, true>), gridDimensions, blockDimensions, m_deviceCalculationStream)(m_inputDataBuffer,
						m_inputDataWidth, m_inputDataHeight, m_inputDataSize, m_inputDataCount, m_paddingX, m_paddingY, m_preactivationGradientsBuffer, m_filterWidth, m_filterHeight,
						m_filterSize, m_numFilters, m_numFilterChannels, m_stride, m_numPatchesX, m_numPatchesY, m_preactivationGradientsPerChunkWidth, m_weightsGradientsPerChunkBuffer);
				}
				else if (m_numFilters % 64 == 0)
				{
	...
	const uint c_numThreadsPerBlock = 128;
	const uint c_filtersBufferLength = (uint)(m_weightsBufferSize / sizeof(float));
	const uint c_numBlocks = DivideUp(c_filtersBufferLength, c_numThreadsPerBlock);
	const uint c_batchSize = m_parallelismMode == ParallelismMode::Model ? m_inputDataCount : m_tierSize * m_inputDataCount;
	LAUNCH_KERNEL_ASYNC(AggregateWeightsGradientsFromChunks, dim3(c_numBlocks), dim3(c_numThreadsPerBlock), m_deviceCalculationStream)(m_weightsGradientsPerChunkBuffer,
		numChunks, c_filtersBufferLength, c_batchSize, m_weightsGradientsBuffer);
	CudaAssert(cudaGetLastError());
}
	...
	else if (m_inputNumChannels == 3)
	{
		...
		if (lastBatch)
		{
			LAUNCH_KERNEL_ASYNC((CalculateDataInputGradients<16, 2, 16, 32, 3, 4, true>), gridDimensions, blockDimensions, m_deviceCalculationStream)(m_preactivationGradientsBuffer,
				m_inputDataWidth, m_inputDataHeight, m_inputDataSize, m_inputDataCount, m_paddingX, m_paddingY, m_weightsBuffer, m_filterWidth, m_filterHeight, m_filterSize,
