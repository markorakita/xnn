	...
	int paddingX, int paddingY, uint unitStride, bool holdsActivationGradients)
{
	...
	m_activationBufferSize = (size_t)m_activationNumChannels * m_activationDataSize * m_inputDataCount * sizeof(float);

	m_holdsActivationGradients = holdsActivationGradients;
}

void MaxPoolLayer::AllocateBuffers(bool allocateTrainBuffers)
{
